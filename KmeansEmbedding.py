...
parser.add_argument('--wsize', default=10, type=int, help="The number of words per sliding window (independent of the index" 
                                            " created, which does not need to be computed again for different sample sizes).")
parser.add_argument('--njobs', default=10, type=int, help="The number of cores assigned to optimize Kmeans.")
...
logging.info("Fitting Kmeans centroids for sparse coding ...")
X_s = Dict(sorted({w: v for w, v in sparse_word_centroids
                    if not v is None}.items(), key=lambda t: len(t[0])))

kmeans = KMeans(n_clusters=args.dim, random_state=0, n_jobs=20)
X = vstack(list(X_s.values()))
kmeans.fit(X)
word_embeddings = X.dot(csr_matrix(kmeans.cluster_centers_).T)
...
logging.info("Shape of resulting embedding matrix: ({}, {})".format(kmeans.cluster_centers_.shape[0], 
...
with open(args.output, "w") as f:
    f.write("{} {}\n".format(len(X_s.keys()), word_embeddings.shape[0]) )
